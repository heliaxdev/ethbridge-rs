...
fn generate_events_crate(
    ...
    let num_dyn_events = events.bridge.len().into_token_stream();
    ...
    let event_codec_impls = [("Bridge", events.bridge)]
        .into_iter()
        .map(|(kind, events)| {
            let kind_events =
            ...
            events
                ...
                .fold(TokenStream::new(), |mut stream, event_ident| {
                    ...
                    stream.extend(quote! {
                        #[doc = #docstring]
                        pub static #codec_ident: DynEventCodec =
    ...
    generate_crate_source(
        ...
        std::iter::once(quote! {
            ...
            pub fn event_codecs() -> [DynEventCodec; #num_dyn_events] {
                [#all_dyn_events]
            }
